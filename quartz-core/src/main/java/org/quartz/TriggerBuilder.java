 ...
 * for you.</p>
 * 《	这个生成器将总是尝试保持在有效状态，为在任何时刻调用build()提供默认设置。例如你不用调用withSchedule（）方法，
 * 		一个默认的马上触发一次的调度将被使用。另一个例子，如果你不调用withIdentity（）方法，触发器名字将为你自动生成	》
 *  
 ...
 * 《	quartz通过DSL提供了创建调度相关实体的内建类型API，DSL最好利用类静态方法导入的优势。	》
 * 
