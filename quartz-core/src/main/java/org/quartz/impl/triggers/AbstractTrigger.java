 ...
 * 《	触发器有一个组和名关联，在每个调度器中组和名进行唯一标识该触发器		》
 * </p>
 * 
 * <p>
...
public abstract class AbstractTrigger<T extends Trigger> implements OperableTrigger {
    ...
    */
     ...
     * triggering (if any).
     * 《	当调度器决定‘触发’触发器（执行相关联的Job）时调用，为了给触发器一次机会更新它自己保证它的下次触发		》
     * </p>
     * 
     ...
     * 《	触发器首次添加到调度器时被调度器调用，为了得到触发器首次触发时间，基于任何关联的日期		》
     * </p>
     * 
     * <p>
     * After this method has been called, <code>getNextFireTime()</code>
     * should return a valid answer.
     * 《	该方法被调用后，getNextFireTime()方法应该返回一个有效的结果	》
     * </p>
     * 
     ...
     * 《	关联于触发器的job被执行后由调度器调用，为了从触发器获取最后的指令		》
     * </p>
     * 
     * @param context
