 ...
 * <--尝试去加载在这个包中被发现的所有ClassLoadHelper类型，当一个被发现的计划执行时，他被提升为下一次类加载时要使用的策略-->
 * 
 * <p>
 ...
 * 'remembers' it.  
 * <--这个方法被使用，因为在Quartz运行的不同环境（不同的应用服务器、独立、手机设备等等）中类加载的行为有着很大的变化。
 * 因为这种不一致，Quartz遇到了一个类加载风格适用所有设计的困难。因此，这个类加载器发现了这个工作方法，所以，记住它。-->
 * </p>
 * 
...
public class CascadingClassLoadHelper implements ClassLoadHelper {
     ...
     * <--对给定的ClassLoadHelper调用一次去初始化自己，包括了偷取类加载器调用线程的机会，那就是初始化quartz的线程-->
     */
