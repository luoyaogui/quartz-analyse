 ...
 * <--这个类是一个线程池的简单实现，基于org.quartz.spi.ThreadPool的接口-->
 * </p>
 * 
 * <p>
 ...
 * <--Runnable对象通过runInThread(Runnable)方法发送到线程池中，锁住直到线程可用-->
 * </p>
 * 
 * <p>
 * The pool has a fixed number of <code>Thread</code>s, and does not grow or
 * shrink based on demand.
 * <--这个池有固定数目的线程，不会因为需求而增长或缩小-->
 * </p>
 * 
 * @author James House
...
public class SimpleThreadPool implements ThreadPool {
    ...
    public void initialize() throws SchedulerConfigException {
        ...
        if (prio <= 0 || prio > 9) {//优先级判断
            throw new SchedulerConfigException(
                    "Thread priority must be > 0 and <= 9");
        }

        if(isThreadsInheritGroupOfInitializingThread()) {//初始化线程的线程组继承关系
            ...
            if (isMakeThreadsDaemons()) {//设置为守护线程
        ...
        while(workerThreads.hasNext()) {//启动工作线程
