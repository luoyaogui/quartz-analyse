...
public interface JobStore {
   ...
   /**
     ...
     * @param noLaterThan 不迟于（noLaterThan If > 0, the JobStore should only return a Trigger that will fire no later than the time represented in this value as milliseconds）
     * @param maxCount	最大批量处理数目
     * @param timeWindow	时间窗口（算法的主要思路是通过采用一个冲突任务替换一个已规划的任务，并将替换任务后移至下一时间窗口或在同一时间窗口内部后移）
     * @return  将被触发的触发器列表
     * @throws JobPersistenceException  
