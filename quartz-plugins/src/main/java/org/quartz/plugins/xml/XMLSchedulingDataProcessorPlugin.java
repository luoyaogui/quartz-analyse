 ...
 * 《如果对JobStoreCMT使用这个插件，请确保设置了wrapInUserTransaction属性为true。
 * 同时，如果你有正数的scanInterval值请确保org.quartz.scheduler.wrapJobExecutionInUserTransaction为true》
 * </p>
 * 
    ...
    public void start(UserTransaction userTransaction) {
        try {
            if (jobFiles.isEmpty() == false) {
                ...
                if (scanInterval > 0) {
                    getScheduler().getContext().put(JOB_INITIALIZATION_PLUGIN_NAME + '_' + getName(), this);//放入上下文中
                }
                
                Iterator<JobFile> iterator = jobFiles.values().iterator();//循环读取所有job文件
                while (iterator.hasNext()) {
                    JobFile jobFile = iterator.next();
                
                    if (scanInterval > 0) {//确定时间间隔可用
                        String jobTriggerName = buildJobTriggerName(jobFile.getFileBasename());//生成唯一的名称
                        TriggerKey tKey = new TriggerKey(jobTriggerName, JOB_INITIALIZATION_PLUGIN_NAME);
                        
                        // 如果有任何相同的则进行移除	remove pre-existing job/trigger, if any
